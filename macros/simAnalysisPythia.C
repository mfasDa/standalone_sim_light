...
class PythiaConstituent : public fastjet::PseudoJet::UserInfoBase {
    public:
        PythiaConstituent(Pythia8::Particle *particle) : 
...
std::vector<fastjet::PseudoJet> select_particles(Pythia8::Event &event) {
    ...
    for(auto partit = event.begin(); partit != event.end(); ++partit) {
        ...
        if(std::abs(particle.eta()) > 0.7) continue;
...
Pythia8::Particle *getPartonOrigin(const fastjet::PseudoJet &recjet, Pythia8::Event &event) {
    ...
    std::sort(partons.begin(), partons.end(), [](const PartonMother &lhs, const PartonMother &rhs) { return lhs.mMotherParticle->e() > rhs.mMotherParticle->e(); });
...
HardProcessType_t getHardProcessType(const Pythia8::Particle *parton) {
    ...
    if(parton->isGluon()) proctype = kGluonJet;
    else if(parton->isDiquark()) proctype = kGluonJet; /// treat diquarks as gluon decays
    else if(parton->isQuark()) proctype = kQuarkJet;
    return proctype;
}

Pythia8::Pythia *configurePythia(int pthardbin, double ecms, int seed) {
    std::array<std::pair<double, double>, 21> pthardbins = {{{0., 5.},  {5., 7.}, {7., 9.}, {9., 12.}, {12., 16.}, {16., 21.}, {21., 28.}, {28., 36.}, {36., 45.},
                                                             {45., 57.}, {57., 70.}, {70., 85.}, {85., 99.}, {99., 115.}, {115., 132.}, {132., 150.}, {150., 169.}, 
                                                             {169., 190.}, {190, 212.}, {212., 235.}, {235., 1000.}}};
    ...
    for(int ievent = 0; ievent < maxevents; ievent++) {
        ...
        for(auto R : ROOT::TSeqI(2, 7)) {
            ...
            for(auto jet : incjets) {
                ...
                if(jet.pt() > 3 * pthard) continue; // outlier cut
